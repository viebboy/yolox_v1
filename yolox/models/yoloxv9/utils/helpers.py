...
from .yolox_utils import bboxes_iou
from .yolox_utils import cxcywh2xyxy
from .yolox_utils import meshgrid
from .yolox_utils import visualize_assign
...
class YOLOXHead(nn.Module):
    def __init__(
        ...
        light = True
        self.set_blocks() if light else self.set_blocks_original()

        self.set_pred_blocks()
        self.use_l1 = False
    ...
    def set_blocks_original(self) -> None:
        Conv = DWConv if self.depthwise else BaseConv
        for i in range(len(self.in_channels)):
            self.stems.append(
                BaseConv(
                    in_channels=int(self.in_channels[i] * self.width),
    ...
    def set_blocks(self) -> None:
        for i in range(len(self.in_channels)):
            self.stems.append(Identity())
            self.cls_convs.append(Identity())
            self.reg_convs.append(Identity())
        return

    def set_pred_blocks(self) -> None:
        for i in range(len(self.in_channels)):
    ...
    def forward(self, xin, labels=None, imgs=None):
        ...
        loss, iou_loss, conf_loss, cls_loss, l1_loss, num_fg = [0.] * 6
        if labels is not None:
        ...
        self.hw = [x.shape[-2:] for x in outputs_anchors]
        # [batch, n_anchors_all, 85]
        outputs_anchors = torch.cat(
            [x.flatten(start_dim=2) for x in outputs_anchors], dim=2,
        ).permute(0, 2, 1)
        if self.decode_in_inference:
            outputs_anchors = self.decode_outputs(
                outputs_anchors, dtype=xin[0].type(),
            )
        ...
        return {
            ...
            'cls_loss': cls_loss,
            'num_fg': num_fg,
            'outputs_anchors': outputs_anchors,
        }
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                try:
                    ...
                    gt_matched_classes = torch.zeros(num_gt).to(device)
                    fg_mask = torch.tensor(
                        [1] * num_gt + [0] * (total_num_anchors - num_gt),
                    ).bool().to(device)
                    pred_ious_this_matching = torch.zeros(num_gt).to(device)
    ...
    def visualize_assign_result(
        ...
        for k, (cls_conv, reg_conv, stride_this_level, x) in enumerate(
            ...
            expanded_strides.append(
                torch.full(
                    (1, grid.shape[1]), stride_this_level,
                ).type_as(
                    xin[0],
                ),
            )
        ...
        nlabel = (labels.sum(dim=2) > 0).sum(dim=1)  # number of objects
        for batch_idx, (img, num_gt, label) in enumerate(
                zip(imgs, nlabel, labels),
        ):
            ...
            img = visualize_assign(
                img, xyxy_boxes, coords, matched_gt_inds, save_name,
            )
