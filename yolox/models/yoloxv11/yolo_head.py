    ...
    def __init__(
        ...
        if groups is not None:
            for i in range(len(in_channels)):
                ...
                self.reg_convs.append(nn.Sequential(*hidden_blocks))
...
class YOLOXHeadDeploy(nn.Module):
    def __init__(
        self,
        input_height,
        input_width,
        ...
        for stride in strides:
            assert input_height % stride == 0
            assert input_width % stride == 0
            h_stride = int(input_height / stride)
            w_stride = int(input_width / stride)
            hw.append((h_stride, w_stride))

        grid_offset , stride_offset = self.compute_offsets(hw, strides)
        self.register_buffer('grid_offset', grid_offset)
        self.register_buffer('stride_offset', stride_offset)
    ...
    def compute_offsets(self, hw, strides):
        ...
        for (hsize, wsize), stride in zip(hw, strides):
            yv, xv = meshgrid([torch.arange(hsize), torch.arange(wsize)])
            grid = torch.stack((xv, yv), 2).view(1, -1, 2)
            grid_offset.append(grid)
            shape = grid.shape[:2]
            stride_offset.append(torch.full((*shape, 1), stride))

        grid_offset = torch.cat(grid_offset, dim=1).float()
        stride_offset = torch.cat(stride_offset, dim=1).float()
        return grid_offset, stride_offset


class YOLOXHeadDeployV2(YOLOXHeadDeploy):
    def forward(self, xin):
        outputs = []
        origin_preds = []
        xin = xin[-self.nb_fpn:]
